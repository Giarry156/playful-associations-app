...
Route::prefix('/associations')->name('associations.')->middleware('auth:sanctum')->group(function () {
    ...
    Route::get('/{association}/users', [AssociationController::class, 'getUsers'])->name('users');
});
        ...
        ->group(function () {
            Route::prefix('/top')->name('top.')->group(function () {
                Route::prefix('/boardgames')->name('boardgames.')->group(function () {
                    Route::get('/', [GameStatsController::class, 'topBoardgame'])->name('ever');
                    Route::get('/associations', [GameStatsController::class, 'topBoardgamesByAssociations'])->name('associations');
                    Route::get('/users', [GameStatsController::class, 'topBoardgamesByUsers'])->name('users');
                });
            });
        });
});
